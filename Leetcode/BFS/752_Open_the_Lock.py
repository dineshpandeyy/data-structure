...
def openLock(deadends, target):
    ...
    def children(lock):
        res = []
        for i in range(4):
            value = str((int(lock[i]) + 1)% 10)
            res.append(lock[:i]+value+lock[i+1:])
            value = str((int(lock[i]) - 1+10) % 10)
            res.append(lock[:i]+value+lock[i+1:])
        return res
    ...
    if "0000" in visited:
        return -1  # fix: early exit if start is in deadends
    visited.add("0000")  # fix: mark start as visited
