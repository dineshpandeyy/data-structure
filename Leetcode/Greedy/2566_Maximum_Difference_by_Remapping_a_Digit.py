...
def minMaxDifference(num: int) -> int:
    ...
    minValueRep = s[0]
    minStr = s.replace(minValueRep, "0")
...
'''
Greedy Strategy:
To maximize: replace the first digit (from the left) that isn’t 9 with 9 in all its occurrences.
To minimize: replace the first digit with 0 in all its occurrences.
...
Space complexity: O(n), due to string manipulations
'''